...
import { TableLinkageView } from '@/components/reits/table-linkage-view';
import { ConsistencyReport } from '@/components/reits/consistency-report';
...
// 八张表定义
const REIT_TABLES = [
  { id: 'product', name: '产品信息', icon: Building2, color: '#667eea', table: 'reit_product_info' },
  { id: 'property', name: '资产信息', icon: MapPin, color: '#764ba2', table: 'reit_property_info' },
  { id: 'financial', name: '财务指标', icon: DollarSign, color: '#10b981', table: 'reit_financial_metrics' },
  { id: 'operational', name: '运营数据', icon: TrendingUp, color: '#f59e0b', table: 'reit_operational_data' },
  { id: 'market', name: '市场表现', icon: BarChart3, color: '#ef4444', table: 'reit_market_performance' },
  { id: 'investor', name: '投资者结构', icon: Users, color: '#8b5cf6', table: 'reit_investor_structure' },
  { id: 'dividend', name: '收益分配', icon: PieChartIcon, color: '#ec4899', table: 'reit_dividend_history' },
  { id: 'risk', name: '风险指标', icon: Shield, color: '#06b6d4', table: 'reit_risk_metrics' },
];
...
export default function REITsDataTablesEnhanced() {
  const router = useRouter();
  const { user, canRead, canUpdate, canExport } = usePermission();
  ...
  const runConsistencyCheck = async () => {
    if (!canRead('reits:all')) return;

    setLoading(true);
    try {
      const currentTable = REIT_TABLES.find(t => t.id === selectedTable)?.table || '';
      const data = selectedProduct ? [selectedProduct] : products;

      const response = await fetch('/api/reits/consistency-check', {
        method: 'POST',
      ...
      if (result.success) {
        setConsistencyResults(result.results);
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mt-0.5">专业数据库 · 全维度数据展示 · 智能联动</p>
                  </div>
                </div>
              </div>
        ...
        <div className="container mx-auto px-4 py-6 space-y-6">
          ...
          {/* 表格联动视图 */}
          {showLinkage && selectedProduct && (
            <TableLinkageView
              sourceTable={REIT_TABLES.find(t => t.id === selectedTable)?.table || ''}
              sourceId={selectedProduct.fund_code}
            />
          )}

          {/* 一致性检查报告 */}
          {showConsistency && selectedProduct && (
            <ConsistencyReport
              results={consistencyResults}
              table={REIT_TABLES.find(t => t.id === selectedTable)?.table || ''}
            />
          )}
                  ...
                  <button
                    ...
                    className={`
                      ...
                      ${selectedTable === table.id
                        ? 'bg-gradient-to-br from-[#667eea]/20 to-[#667eea]/10 border-2 border-[#667eea] shadow-lg'
                        : 'bg-slate-700/30 border border-slate-600/50 hover:bg-slate-700/50'
                      }
                    `}
                  >
                    <table.icon className={`w-6 h-6 mb-2 ${selectedTable === table.id ? 'text-[#667eea]' : 'text-slate-400'}`} />
                  ...
                  <div className="text-center py-20">
                    <div className="flex flex-col items-center gap-4">
                      <div className="w-16 h-16 bg-slate-700/50 rounded-full flex items-center justify-center">
                        {(() => {
                          const table = REIT_TABLES.find(t => t.id === selectedTable);
                          return table ? <table.icon className="w-8 h-8 text-slate-400" /> : null;
                        })()}
                      </div>
                      <h3 className="text-xl font-semibold text-white">
                        {REIT_TABLES.find(t => t.id === selectedTable)?.name}
                      </h3>
