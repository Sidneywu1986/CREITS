...
import { AGENTS, MARKET_DATA } from '../src/types';
...
export default function Home() {
  return (
    <MainLayout>
      ...
      {/* 快速入口 */}
      <div className="mt-8">
        <h3 className="text-2xl font-bold mb-6">快速入口</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Link href="/issuance-status">
            <Card className="hover:shadow-xl transition-all duration-300 cursor-pointer border-2 hover:border-blue-600">
              <CardContent className="pt-6">
                <h4 className="font-semibold mb-2">发行状态跟踪</h4>
                <p className="text-sm text-muted-foreground">实时跟踪REITs和ABS产品从申请到上市的全过程</p>
              </CardContent>
            </Card>
          </Link>
          <Link href="/issued-reits">
            <Card className="hover:shadow-xl transition-all duration-300 cursor-pointer border-2 hover:border-purple-600">
              <CardContent className="pt-6">
                <h4 className="font-semibold mb-2">已发行REITs</h4>
                <p className="text-sm text-muted-foreground">查看市场上已发行的REITs产品实时行情</p>
              </CardContent>
            </Card>
          </Link>
          <Link href="/issued-abs">
            <Card className="hover:shadow-xl transition-all duration-300 cursor-pointer border-2 hover:border-green-600">
              <CardContent className="pt-6">
                <h4 className="font-semibold mb-2">已发行ABS</h4>
                <p className="text-sm text-muted-foreground">查看已发行的ABS项目信息和票面利率</p>
              </CardContent>
            </Card>
          </Link>
        </div>
