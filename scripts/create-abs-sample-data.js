  ...
  return [
    {
      ...
      currentBalance: scale * seniorARatio,
      couponType: '固定',
...
function generateCollateralPool(productCode, scale) {
  return {
    pool_id: `POOL_${productCode}`,
    product_code: productCode,
    report_date: '2024-12-31',
    total_principal_balance: scale * 0.95,
    total_asset_count: 100 + Math.floor(Math.random() * 900),
    avg_loan_size: (scale * 0.95) / (100 + Math.floor(Math.random() * 900)),
    weighted_avg_maturity: 24 + Math.floor(Math.random() * 36),
    weighted_avg_loan_age: 6 + Math.floor(Math.random() * 18),
    weighted_avg_interest_rate: 4 + Math.random() * 4,
    top1_borrower_ratio: Math.random() * 5,
    top5_borrower_ratio: 5 + Math.random() * 15,
    top1_region_ratio: Math.random() * 10,
    herfindahl_index: 0.01 + Math.random() * 0.05,
    weighted_avg_credit_score: 650 + Math.floor(Math.random() * 100),
    ltv_avg: 50 + Math.random() * 30,
    delinquency_30plus: Math.random() * 2,
    delinquency_60plus: Math.random() * 1,
    delinquency_90plus: Math.random() * 0.5,
    cumulative_default_rate: Math.random() * 1,
    cumulative_prepayment_rate: Math.random() * 5,
    cpr: Math.random() * 10,
    cumulative_recovery_rate: 60 + Math.random() * 30,
    recovery_lag_months: 3 + Math.floor(Math.random() * 6),
  };
}
...
function generateLoanDetails(productCode, poolId, count = 10) {
  ...
  for (let i = 0; i < count; i++) {
    loans.push({
      loan_id: `LOAN_${productCode}_${String(i + 1).padStart(4, '0')}`,
      product_code: productCode,
      pool_id: poolId,
      report_date: '2024-12-31',
      origination_date: '2022-01-01',
      maturity_date: '2026-01-01',
      original_balance: 50 + Math.random() * 450,
      current_balance: 30 + Math.random() * 350,
      interest_rate: 4 + Math.random() * 5,
      rate_type: Math.random() > 0.3 ? '固定' : '浮动',
      borrower_type: Math.random() > 0.3 ? '个人' : '企业',
      borrower_industry: industries[Math.floor(Math.random() * industries.length)],
      borrower_region: regions[Math.floor(Math.random() * regions.length)],
      credit_score: 600 + Math.floor(Math.random() * 200),
      debt_to_income: Math.random() * 50,
      collateral_type: Math.random() > 0.3 ? '房产' : '其他',
      collateral_value: 80 + Math.random() * 820,
      ltv_origination: 50 + Math.random() * 40,
      ltv_current: 45 + Math.random() * 40,
      collateral_region: regions[Math.floor(Math.random() * regions.length)],
      payment_status: Math.random() > 0.95 ? '逾期' : '正常',
      days_past_due: Math.random() > 0.95 ? Math.floor(Math.random() * 90) : 0,
      delinquency_status: Math.random() > 0.95 ? 'D30' : 'D0',
      modification_flag: Math.random() > 0.98,
      forbearance_flag: Math.random() > 0.99,
