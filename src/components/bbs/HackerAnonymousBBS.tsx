...
export default function HackerAnonymousBBS({ className }: HackerAnonymousBBSProps) {
  ...
  const generateAnonymousNickname = useCallback((): string => {
    const adjectives = ['神秘', '沉默', '暗影', '幽灵', '影子', '未知', '隐形', '神秘人', '观察者', '匿名'];
    const nouns = ['黑客', '行者', '访客', '探索者', '思考者', '特工', '观察者', '隐士', '流浪者', '幻影'];
  ...
  return (
    <div className={`hacker-bbs ${className}`}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-green-400 font-mono text-lg">
                  <span className="text-green-500">$</span> ANONYMOUS_BBS
                </CardTitle>
                <CardDescription className="text-green-600/70 font-mono text-xs">
                  隐私保护 · 临时讨论 · 数据不保存
                </CardDescription>
              </div>
            </div>
        ...
        <CardContent className="p-0">
            ...
            <div className="p-6 border-b border-green-500/20 bg-green-950/20">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1">
                  <h3 className="text-green-400 font-mono text-sm mb-1">创建匿名讨论主题</h3>
              ...
              <div className="space-y-3">
                <Input
                  ...
                  className="bg-black/50 border-green-500/30 text-green-400 placeholder-green-600/50 font-mono text-sm focus:border-green-500"
                  maxLength={100}
                />
                <Textarea
                  ...
                  className="bg-black/50 border-green-500/30 text-green-400 placeholder-green-600/50 font-mono text-sm min-h-[80px] resize-none focus:border-green-500"
                  maxLength={500}
                />
          ...
          {!showTopics && !selectedRoom && topics.length === 0 && (
            <div className="p-12 flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-green-400 font-mono text-lg mb-2">隐私保护匿名讨论区</h3>
              <p className="text-green-600/60 text-sm mb-6 max-w-md">
            ...
            <div className="p-4">
              {topics.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-green-600/50 font-mono text-sm">暂无讨论主题</p>
                </div>
              ) : (
                <ScrollArea className="h-[600px] pr-4">
                  <div className="space-y-3">
                    {topics.map((topic) => (
                      <Card
                        key={topic.id}
                        className="border-green-500/20 bg-black/50 hover:bg-green-950/20 hover:border-green-500/40 transition-all cursor-pointer"
                        onClick={() => {
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-2">
                                <h4 className="text-green-400 font-mono text-sm font-semibold truncate">
                                  {topic.title}
                                </h4>
                                {topic.isActive && (
                                  <Badge className="bg-green-500/10 border-green-500/30 text-green-500 font-mono text-xs">
                                    <Activity className="w-3 h-3 mr-1" />
                                    活跃
                                  </Badge>
                                )}
                              ...
                              {topic.description && (
                                <p className="text-green-600/60 text-xs mb-2 line-clamp-2">
                                  {topic.description}
                                </p>
                              )}
                              <div className="flex items-center gap-3 text-xs">
                                <span className="text-green-600/50 font-mono">
                                  by {topic.author.nickname}
                                </span>
                                <span className="text-green-600/30">·</span>
                                <span className="text-green-600/50 flex items-center gap-1">
          ...
          {selectedRoom && !showTopics && (
            <div className="flex flex-col h-[600px]">
              {/* 聊天室头部 */}
              <div className="p-4 border-b border-green-500/20 bg-green-950/10">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Button
                      variant="ghost"
                    ...
                    <div>
                      ...
                      <p className="text-green-600/50 text-xs font-mono">
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {selectedRoom.messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center">
                    <div className="w-12 h-12 rounded-full bg-green-500/10 flex items-center justify-center mb-3 border border-green-500/30">
                      <MessageSquare className="w-6 h-6 text-green-500" />
                    </div>
                    <p className="text-green-600/50 font-mono text-sm mb-1">暂无消息</p>
                    <p className="text-green-600/30 text-xs">开始讨论吧，所有消息都是端到端加密的</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {selectedRoom.messages.map((message) => (
                      ...
                      >
                        ...
                        >
                          <div className="text-xs text-green-600/50 mb-1 font-mono">
              ...
              {/* 输入区域 */}
              <div className="p-4 border-t border-green-500/20 bg-green-950/10">
                <div className="flex gap-3">
                  <Input
                    placeholder="输入消息..."
                    value={messageInput}
                ...
                <div className="flex items-center gap-2 mt-2 text-xs text-green-600/50">
      ...
      {/* 隐私保护说明 */}
      <Card className="mt-4 border-green-500/20 bg-black/90 backdrop-blur-sm">
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            <AlertTriangle className="w-5 h-5 text-yellow-500 flex-shrink-0 mt-0.5" />
            <div className="flex-1">
              <h4 className="text-yellow-500 font-mono text-sm font-semibold mb-2">隐私保护机制</h4>
              <ul className="text-green-600/60 text-xs space-y-1 font-mono">
