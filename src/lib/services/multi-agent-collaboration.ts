...
export class QuestionAnalyzer {
   ...
   * 包含常见问候语及其变体
   */
  private static readonly GREETING_BLACKLIST = new Set([
    // 基础问候
    '你好', '您好', 'hello', 'hi', '哈喽', '嗨',
    // 时间问候
    '早上好', '下午好', '晚上好', '晚安',
  ...
  private static readonly SYNONYM_MAP: Record<string, string[]> = {
    '经营': ['运营', '管理', '经营分析', '运营分析', '运营管理', '运营指标', '运营效率'],
    '分析': ['评估', '判断', '研究', '分析判断'],
    '判断': ['分析', '评估', '判断分析'],
    '趋势': ['走势', '方向', '发展', '前景', '后期趋势'],
    '后期': ['未来', '后续', '后期发展'],
    ...
    // 新增同义词
    '分配率': ['分红率', '派息率', '收益率分配', '现金分派率'],
    '收益率': ['回报率', '投资回报', '年化收益率', '预期收益率'],
    '分红': ['派息', '现金分红', '收益分配', '利润分配'],
    '派息': ['分红', '现金派息', '收益派发'],
    '净值': ['基金净值', '资产净值', '每股净值', 'NAV'],
    '估值': ['评估', '资产估值', '价值评估', '估值分析'],
    '投资': ['投资价值', '投资回报', '投资收益', '投资分析'],
    '收益': ['收益分配', '可分配收益', '收益分析', '收益预测'],
    '现金流': ['现金流折现', 'DCF', '现金流量', '现金流入流出'],
    '发行': ['发行价', '发行定价', '发行规模', '发行数量']
  };
  ...
  private static isGreeting(question: string): boolean {
    ...
    // 1. 精确匹配黑名单（不含标点）
    if (this.GREETING_BLACKLIST.has(trimmed)) {
      return true;
    }
